...
def parse_article_info(medline_doi, medline_art, year_info_only, nlm_category, time_start):
...
#    print(medline[0])
    ...
    if not title or not abstract or not doi :
        ...
        dict_out = {'title': title,
                    ...
                    'abstract': tm.clean_abstract(abstract,False),
                    ...
                    'cpds':"",
                    'scores':"",
                    'indi':random.randint(1,2),
                    'udate': time_start,
                    'cpdsdate': datetime.datetime.now(),
                    'aopdate': datetime.datetime.now(),
                    }
        return dict_out



def parse_medline_xml(path,time_start, year_info_only=True, nlm_category=False):
    ...
    for i in range(len(medline[0])):
        article_info = parse_article_info(medline[0][i], medline[1][i], year_info_only, nlm_category,time_start)
        if article_info:
...
def get_abstracts_from_pubmedfile(abstracts_file):
    """ insere le fichier xml 'abstracts_file' dans la bd 'abstracts_db'
    """
    dict_xml = parse_medline_xml(abstracts_file, 0)
